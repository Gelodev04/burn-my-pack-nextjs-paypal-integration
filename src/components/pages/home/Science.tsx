...
export const ScienceSection = () => {
  ...
  return (
    <section className='py-7 px-4'>
        <h1 className='text-center text-[20px] font-bold text-[#2D2D2D]'>The Science Behind Financial Punishment</h1>

        <ul className='mt-9 flex lg:flex-row flex-col gap-[2rem] max-w-[250px] mx-auto '>
            {scienceData.map((item, index) => (
                <li key={index} className='flex flex-col  gap-2 bg-[#F8F8F8] pt-[2.2rem] px-[2rem] pb-[1rem]  rounded-lg'>
                    <Image className='w-[30px] h-[30px]' src={item.icon} alt={item.title} width={50} height={50} />
                    <div className='space-y-2 mt-1'>
                        <h2 className='text-[15px] font-bold text-[#2D2D2D] '>{item.title}</h2>
                        <p className='text-[11px] text-[#2D2D2D] '>{item.description}</p>
                    </div>
                </li>
            ))}
        </ul>
